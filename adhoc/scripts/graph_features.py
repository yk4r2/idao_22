...
def _extract_features(data):
    for i, _ in data.iterrows():
        ...
        data.loc[i, "distance_min"] = np.min(
            ...
            + np.eye(structure.num_sites) * data.loc[i, "distance_max"]
        )

        data.loc[i, "num_edges"] = len(graph.edges)
        data.loc[
            i, "degree_assortativity_coefficient"
        ...
        for e in ["Mo", "S", "Se", "W"]:
            selected_nodes = [n for n, v in graph.nodes(data=True) if v["element"] == e]
            ...
            data.loc[i, "neighbor_degree_min_" + e] = np.min(neighbor_degrees_e)
            data.loc[i, "neighbor_degree_mean_" + e] = np.mean(neighbor_degrees_e)
            data.loc[i, "neighbor_degree_max_" + e] = np.max(neighbor_degrees_e)
        ...
        adamic_adars = list(
            map(lambda x: x[2], list(link_prediction.adamic_adar_index(graph)))
        )
...
def main():
    ...
    formula_statistics = dict(
        df_public.groupby(by="formula")["band_gap"].agg(["min", "max", "mean"])
    )
    ...
    for i, _ in df_public.iterrows():
        ...
        for name in formula_statistics:
            df_public.loc[i, name + "_formula_band_gap"] = formula_statistics[name].get(
                formula, 0
            )
    ...
    for i, _ in df_private.iterrows():
        ...
        for name in formula_statistics:
            df_private.loc[i, name + "_formula_band_gap"] = formula_statistics[
                name
            ].get(formula, 0)
